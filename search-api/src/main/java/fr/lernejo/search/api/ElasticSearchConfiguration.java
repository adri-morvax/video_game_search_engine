...
public class ElasticSearchConfiguration {
    ...
    public RestHighLevelClient client (@Value("${elasticsearch.host:localhost}") String address,
                                       ...
                                       @Value("${elasticsearch.username:elastic}") String name,
                                       @Value("${elasticsearch.password:admin}") String pass){
        final CredentialsProvider credentials = new BasicCredentialsProvider();
        credentials.setCredentials(AuthScope.ANY, new UsernamePasswordCredentials(name, pass));
